...
import static com.shanks.myapp.utils.Utils.userStatus;
...
//    SEND_EMAIL_SMS
    ...
    private void getSmsAndEmailContent(String offerid) {
        String url = Utils.BASE_URL + Utils.SEND_EMAIL_SMS
                + "?offerid=" + offerid
                + "&communicateuser=y";
        CallService service = new CallService(Refer.this, url, Utils.GET, true, new CallService.OnServicecall() {
            @Override
    ...
    private void init() {
        ...
        session = Session.getSession(Refer.this);
...
//                dealernameString = dealername.getText().toString().trim();
    ...
    private void saveLeads(String nameSting, String phonenumberString, final String emailString, String planbuyString,
        ...
        CallService service = new CallService(Refer.this, str, Utils.GET, true, new CallService.OnServicecall() {
            @Override
            public void onServicecall(String response) {

                    Log.d("ankit1", "::::" + response);
                ...
                try {
                    ...
                    if (responseCode.equalsIgnoreCase("2001")) {
                        ...
                        Toast.makeText(Refer.this, "Lead not generated please try again", Toast.LENGTH_LONG).show();
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
...
//        sendToHome();
    ...
    private void showToast(String message) {
        Toast.makeText(Refer.this, message, Toast.LENGTH_LONG).show();
    }
    private void getAllCities() {
        String url = Utils.BASE_URL + Utils.GET_ALL_CITIES;
        CallService service = new CallService(Refer.this, url, Utils.GET, false, new CallService.OnServicecall() {
            @Override
            public void onServicecall(String response) {
                try {
                    PARSE_JSON(response);
    ...
    private void PARSE_JSON(String response) throws Exception {
        ...
        //spinner_rows
        OfferCityAdapter adapter = new OfferCityAdapter(Refer.this, R.layout.spinner_rows, cityArray);
...
//        String url = Utils.BASE_URL+Utils.GET_DEALER_LIST+"?preferredLocation="+city_id;
    ...
    private void PARSE_DEALER_JSON(String response) throws Exception {
        ...
        DealerNameAdapter adapter = new DealerNameAdapter(Refer.this, 0, DealerNameModels, getResources());
...
//        planbuy
        ...
        PlanToBuyAdapter adapter = new PlanToBuyAdapter(Refer.this, 0, planToBuyModels, getResources());
...
//                "&rpt=1";
        ...
        String url1 = "http://transactional.msgadvert.com/api/swsendSingle.asp" +
                "?username=t1SkillWorld&password=29271485" +
                "&sender=SKILWD" +
                "&sendto=" +phonenumber.getText().toString()+
                "&message="+smsContent;

        Log.wtf("ankit", url1);
        CallService service = new CallService(Refer.this, url1, Utils.GET, true, new CallService.OnServicecall() {
            @Override
            public void onServicecall(String response) {
    ...
    private void sendEmail(final String emailString,final String emailSubject,final String emailContent){
        new AsyncTask<Void,Void,Void>(){
            ...
            protected void onPreExecute() {
                super.onPreExecute();
                pd = ProgressDialog.show(Refer.this,"Loading...","Please wait");
            }
            ...
            protected Void doInBackground(Void... voids) {
                try {
                    GMailSender sender = new GMailSender("info@skillworld.in", "@swn2016@");
