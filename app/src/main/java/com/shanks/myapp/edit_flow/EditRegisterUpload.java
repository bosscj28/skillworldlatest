...
public class EditRegisterUpload  extends AppCompatActivity{
    ...
    private void init(){
        ...
        session = Session.getSession(EditRegisterUpload.this);
        header = (TextView)findViewById(R.id.header);
        header.setText("UPLOAD IMAGES");
...
//                startActivity(new Intent(EditRegisterUpload.this,EditRegistrationBankDetails.class));
                if(adhar_serverFilePath.equalsIgnoreCase("")){
                    Utils.makeDialog(EditRegisterUpload.this,"Please select adhar card");
                } else {
                    ...
                    CallService service = new CallService(EditRegisterUpload.this, url, Utils.GET, true, new CallService.OnServicecall() {
                        @Override
...
//                            {"result":"Success.","userid":"182","responseCode":"2001"}
                            ...
                            try{
                                ...
                                if(responseCode.equalsIgnoreCase("2001")){
                                    startActivity(new Intent(EditRegisterUpload.this, EditRegistrationBankDetails.class));//RegistrationBankDetails.class));
                                    finish();
                                }
                            } catch (Exception ex){
                                ex.printStackTrace();
    ...
    private void getDetails(){
        String url = Utils.BASE_URL+Utils.UPLOAD_DETAILS + "?userid="+session.getUserId();
        CallService service = new CallService(EditRegisterUpload.this, url, Utils.GET, true, new CallService.OnServicecall() {
            @Override
            public void onServicecall(String response) {
                Log.d("ankit",response);
    ...
    private void PARSE_DATA(String response)throws Exception{
        JSONObject jobj = new JSONObject(response);
        JSONArray documentdetailList = jobj.getJSONArray("documentdetailList");

        for (int i=0;i<documentdetailList.length();i++){
            JSONObject innerJob = documentdetailList.getJSONObject(i);

            String documentName = innerJob.getString("documentName");
            if(documentName.equalsIgnoreCase("Pan Card")){

                pan_serverFilePath = innerJob.getString("documentPath");
                final String  ImagePath = Utils.BASE_IMAGE_URL+ pan_serverFilePath;
                Picasso.with(EditRegisterUpload.this).load(ImagePath).into(pan_image);

            } else if(documentName.equalsIgnoreCase("Passport")){

                marksheet_serverFilePath = innerJob.getString("documentPath");
                final String  ImagePath = Utils.BASE_IMAGE_URL+ marksheet_serverFilePath;
                Picasso.with(EditRegisterUpload.this).load(ImagePath).into(marksheet_image);

            } else if(documentName.equalsIgnoreCase("Aadhar Card")){

                adhar_serverFilePath = innerJob.getString("documentPath");
                final String  ImagePath = Utils.BASE_IMAGE_URL+ adhar_serverFilePath;
                Picasso.with(EditRegisterUpload.this).load(ImagePath).into(adhar_image);
    ...
    private void makeDialog(final String image_for){
        // custom dialog
        final Dialog dialog = new Dialog(EditRegisterUpload.this);
    ...
    private String setAndSendFromCamera(final Intent data,final CircleImageView imageView,final String filePath){
        ...
        try {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Picasso.with(EditRegisterUpload.this).load(ImagePath).into(imageView);
                }
            });
    ...
    private String setAndSendFromSdcard(Intent data,final CircleImageView imageView){
        ...
        synchronized (this){
            if (Build.VERSION.SDK_INT < 11)
                selectedImagePath = getRealPathFromURI_BelowAPI11(EditRegisterUpload.this, selectedImageUri);

                // SDK >= 11 && SDK < 19
            else if (Build.VERSION.SDK_INT < 19)
                selectedImagePath = getRealPathFromURI_API11to18(EditRegisterUpload.this, selectedImageUri);

                // SDK > 19 (Android 4.4)
            else
                selectedImagePath = getRealPathFromURI_API19(EditRegisterUpload.this, selectedImageUri);
        }
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                imageView.setRotation(getCameraPhotoOrientation(EditRegisterUpload.this,selectedImageUri,selectedImagePath));
            }
        });
